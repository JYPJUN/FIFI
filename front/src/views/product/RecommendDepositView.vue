<template>
  <div class="container">
    <h1 style="text-align: center;">예금 추천</h1>
    <h4 style="position: relative; left: 30px;">⁕ 금융 상품 추천은 나이, 연봉, 회사 선호도, 상품 가입 유저 기준으로 산출됩니다.</h4>
    <table style="margin: 30px;">
      <thead>
        <tr>
          <th @click="sortBy('deposit_company')">
            금융상품명
            <span v-if="currentSort === 'deposit_company'">
              <i :class="currentSortDir === 'asc' ? 'arrow up' : 'arrow down'"></i>
            </span>
          </th>
          <th @click="sortBy('deposit_name')">
            금융회사명
            <span v-if="currentSort === 'deposit_name'">
              <i :class="currentSortDir === 'asc' ? 'arrow up' : 'arrow down'"></i>
            </span>
          </th>
          <th @click="sortBy('age')">
      ...
      <tbody>
        <tr v-for="record in sortedRecords" :key="record.id" @click="goToDetail(record.id)">
          <td>{{ record.deposit_name }}</td>
          <td>{{ record.deposit_company }}</td>
...
export default {
  ...
  methods: {
    fetchRecords() {
      ...
      axios.get(`${API_URL}/api/v1/products/deposit/getrecommendproduct/`, {
    ...
    goToDetail(id) {
      this.$router.push({ name: 'TimeDepositDetailView', params: { id } });
    },
    formatValue(value) {
