...
onMounted(() => {
  store.getProduct().then((response) => {
    saving_detail.value = store.savingall
    saving_detail.value.forEach(product => {
...
const uniqueCompanies = computed(() => {
  const companies = saving_detail.value.map(product => product.kor_co_nm);
  return [...new Set(companies)];
});

const filteredProducts = computed(() => {
  if (selectedCompany.value) {
    return saving_detail.value.filter(product => product.kor_co_nm === selectedCompany.value);
  }
  return saving_detail.value;
});
