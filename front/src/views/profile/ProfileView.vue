<template>
  <div class="container">
    ...
    <div v-if="!isProfileView" class="interest-view">
      <h1>좋아요한 게시글</h1>
      <div v-if="!likedArticles || likedArticles.length === 0" class="no-articles">
        <p>좋아요 한 게시글이 없습니다</p>
      </div>
      <div v-else>
        <div class="article-cards">
          <div v-for="article in likedArticles" :key="article.id" class="article-card">
            <RouterLink :to="{ name: 'DetailView', params: { id: article.id } }" class="link-font">
            <h2>{{ article.title }}</h2>
            <p>{{ article.content }}</p>
      ...
      <div class="profile-container">
        <div v-if="detail" class="profile-details">
          ...
          <div class="profile-info">
            ...
            <p class="info-item"><span class="label">닉네임:</span> {{ detail.nickname }}</p>
            <p class="info-item"><span class="label">이메일:</span> {{ detail.email }}</p>
            <p class="info-item"><span class="label">생일:</span> {{ detail.birth_date }}</p>
            <p class="info-item" v-if="detail.income"><span class="label">연봉:</span> {{ formattedIncome }}</p>
            <p class="info-item" v-else><span class="label">연봉:</span> 연봉을 추가해주세요</p>
          </div>
...
export default {
  ...
  setup() {
    ...
    const formatIncome = (income) => {
      ...
      if (isNaN(incomeNumber)) return income;
      if (incomeNumber >= 10000) {
        const billion = Math.floor(incomeNumber / 10000);
        const million = incomeNumber % 10000;
        return `${billion}억 ${million > 0 ? million + '만원' : ''}`;
      }
      return `${incomeNumber}만원`;
    };

    watchEffect(() => {
      detail.value = store.userdetail;
      if (detail.value && detail.value.income) {
        formattedIncome.value = formatIncome(detail.value.income);
      }
    });
